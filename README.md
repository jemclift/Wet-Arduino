...
Unfortunately, as most university students do not live in their university accommodation all year round, they often leave their plants unattended, unable to water them. Students are also busy, so can forget to water their plants.
...
To solve this problem, our project is a plant irrigation system that automatically waters a plant when its soil is too dry. It also monitors other important values like light and temperature, and all this data is displayed on a remote Arduino with an LCD that can be placed on your desk. If the plant's soil is too dry, it is too hot or cold, a warning is displayed with a buzzer tone and flashing LED to warn the plant's owner.
...
A transistor was used to power the pump. The emitter was connected a source of 5V, The collector was connected straight to ground, the base was connected to pin 3 and was in series with a 10k Ohms resistor. This meant that in order to water the Arduino you would just need to send a voltage through pin 3.

#### Description
One Arduino is placed at the plant and the other is intended to display information and warnings about the plant, so can be placed on a desk or somewhere insight of the user. The boards are connected by 3 wires, which can be long enough to place the master and display far apart in a room.
...
- We would use a better soil moisture sensor. This is because the sensor we use degrades over time and can actually poison any plants or even kill them if the sensor is used over a prolonged period of time.
...
- It would have also been beneficial to have more testing time to allow us to fine tune the watering system. We could have also then developed a more complex system that would choose an appropriate moisture level for different types of plants.
